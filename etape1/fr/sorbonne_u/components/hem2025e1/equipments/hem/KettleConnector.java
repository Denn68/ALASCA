...
implements	AdjustableCI
{
	 ...
	 *  6 (2000.0 watts).													*/
	public static final int		MAX_KETTLE_MODE = 6;
	/** the minimum admissible temperature from which the kettle should
	 *  be resumed in priority after being suspended to save energy.		*/
	public static final double	MIN_KETTLE_ADMISSIBLE_TEMP = 12.0;
	/** the maximal admissible difference between the target and the
	 *  current temperature from which the kettle should be resumed in
	 *  priority after being suspended to save energy.						*/
	public static final double	MAX_KETTLE_ADMISSIBLE_DELTA = 10.0;

	/** the current mode of the kettle.										*/
	protected int		currentMode;
	/** true if the kettle has been suspended, false otherwise.				*/
	protected boolean	isSuspended;

	public				KettleConnector()
	{
		super();
		this.currentMode = MAX_KETTLE_MODE;
		this.isSuspended = false;
	}

	protected double	computePowerLevel(int mode) throws Exception
	{
		assert	mode > 0 || mode <= MAX_KETTLE_MODE :
				new PreconditionException("mode > 0 || mode <= MAX_MODE");

		double maxPowerLevel =
				((KettleExternalControlJava4CI)this.offering).
													getMaxPowerLevelJava4();
		return (mode - 1) * maxPowerLevel/(MAX_KETTLE_MODE - 1);
	}
	...
	public boolean		upMode() throws Exception
	{
		...
		assert	this.currentMode() < MAX_KETTLE_MODE :
	...
	public double		emergency() throws Exception
	{
		...
		double ret = -1.0;
		if (currentTemperature < KettleConnector.MIN_KETTLE_ADMISSIBLE_TEMP ||
							delta >= KettleConnector.MAX_KETTLE_ADMISSIBLE_DELTA) {
			ret = 1.0;
		} else {
			ret = delta/KettleConnector.MAX_KETTLE_ADMISSIBLE_DELTA;
		}
