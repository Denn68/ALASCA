...
extends AbstractCVM
{
	...
	public static Duration SIMULATION_DURATION =
			new Duration(10.0, SIMULATION_TIME_UNIT);
	/** Acceleration factor. */
...
//			ExecutionMode.STANDARD;
					...
					owner -> {
						try {
							((WashingMachineTesterCyPhy)owner).getWmop()
								.startWashing(
									30L,
									new Measure<Double>(40.0, WashingMachineTemperatureI.TEMPERATURE_UNIT));
						} catch (Exception e) {
							throw new BCMRuntimeException(e);
						}
					}),
	...
	throws VerboseException
	{
		...
		Instant switchOnInstant = startInstant.plusSeconds(1800); 
		Instant startWashingInstant = startInstant.plusSeconds(3600); 
		Instant switchOffInstant = startInstant.plusSeconds(d - 1800);  
		...
		return new TestScenarioWithSimulation(
			...
			new TestStepI[] {
				new TestStep(
					...
					owner -> {
						try {
							System.out.println(">>> TEST STEP 1: switchOn executing at " + java.time.Instant.now());
							((WashingMachineTesterCyPhy)owner).getWmop().switchOn();
							System.out.println(">>> TEST STEP 1: switchOn completed");
						} catch (Exception e) {
							System.out.println(">>> TEST STEP 1: switchOn FAILED: " + e.getMessage());
							throw new BCMRuntimeException(e);
						}
					}),
				new TestStep(
						...
						owner -> {
							try {
								System.out.println(">>> TEST STEP 2: startWashing executing at " + java.time.Instant.now());
								((WashingMachineTesterCyPhy)owner).getWmop().startWashing(10L,
										new Measure<Double>(
												21.0,
												WashingMachineTemperatureI.TEMPERATURE_UNIT));
								System.out.println(">>> TEST STEP 2: startWashing completed");
							} catch (Exception e) {
								System.out.println(">>> TEST STEP 2: startWashing FAILED: " + e.getMessage());
								throw new BCMRuntimeException(e);
							}
						}),
				new TestStep(
					...
					owner -> {
						try {
							System.out.println(">>> TEST STEP 3: switchOff executing at " + java.time.Instant.now());
							((WashingMachineTesterCyPhy)owner).getWmop().switchOff();
							System.out.println(">>> TEST STEP 3: switchOff completed");
						} catch (Exception e) {
							System.out.println(">>> TEST STEP 3: switchOff FAILED: " + e.getMessage());
							throw new BCMRuntimeException(e);
						}
					})
			});
