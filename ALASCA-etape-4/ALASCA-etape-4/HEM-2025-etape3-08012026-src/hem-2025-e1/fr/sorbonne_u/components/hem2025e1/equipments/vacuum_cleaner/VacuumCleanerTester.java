...
// -----------------------------------------------------------------------------
/**
 * The class <code>VacuumCleanerTester</code> implements a component performing
 * tests on the vacuum cleaner component.
 *
 * <p>
 ...
 * 
 * <p>
 * The vacuum cleaner is an <b>uncontrollable</b> appliance with only two modes:
 * LOW and HIGH. This tester verifies all the basic operations of the vacuum
 * cleaner component.
 * </p>
 * 
 * @author Team
...
public class VacuumCleanerTester
	...
	protected static boolean implementationInvariants(VacuumCleanerTester vct) {
		...
		ret &= AssertionChecking.checkImplementationInvariant(
				...
				VacuumCleanerTester.class, vct,
				"vct.vacuumCleanerInboundPortURI != null && "
						+ "!vct.vacuumCleanerInboundPortURI.isEmpty()");
		return ret;
	}

	protected static boolean invariants(VacuumCleanerTester vct) {
		assert vct != null : new PreconditionException("vct != null");
	...
	protected void initialise(
		...
		assert implementationInvariants(this) : new ImplementationInvariantException(
				"VacuumCleanerTester.implementationInvariants(this)");
		assert invariants(this) : new InvariantException("VacuumCleanerTester.invariants(this)");
	}
	...
	public void testGetMode() {
		...
		try {
			...
			result = this.vcop.getMode();
			this.logMessage("    Then the vacuum cleaner is in low mode");
			if (!VacuumCleanerMode.LOW.equals(result)) {
				this.logMessage("     but was: " + result);
				this.statistics.incorrectResult();
			}
		} catch (Throwable e) {
	...
	public void testTurnOnOff() {
		...
		try {
			...
			this.logMessage("    And the vacuum cleaner is in low mode");
	...
	public void testSetLowHigh() {
		this.logMessage("Feature: switching the vacuum cleaner between low and high.");
