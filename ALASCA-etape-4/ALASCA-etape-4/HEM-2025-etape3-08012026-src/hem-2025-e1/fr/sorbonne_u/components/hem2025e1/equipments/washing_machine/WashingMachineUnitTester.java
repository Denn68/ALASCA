...
public class WashingMachineUnitTester
	...
	protected WashingMachineUnitTester(
			...
			String washingMachineExternalControlInboundPortURI) throws Exception {
		super(1, 1);
	...
	protected void initialise(
		...
		this.washingMachineInternalControlInboundPortURI = washingMachineInternalControlInboundPortURI;
		this.wmicop = new WashingMachineInternalControlOutboundPort(this);
		this.wmicop.publishPort();
		this.washingMachineExternalControlInboundPortURI = washingMachineExternalControlInboundPortURI;
		...
		assert WashingMachineUnitTester.implementationInvariants(this) : new ImplementationInvariantException(
	...
	protected void testTargetTemperature() {
		...
		try {
			...
			if (temperature.getMeasure().getData() == WashingMachine.STANDARD_TARGET_TEMPERATURE.getData()
	...
	protected void testCurrentTemperature() {
		...
		try {
			...
			if (temperature.getMeasure().getData()
					.equals(WashingMachine.FAKE_CURRENT_TEMPERATURE.getMeasure().getData()) &&
					temperature.getMeasure().getMeasurementUnit()
							.equals(WashingMachine.FAKE_CURRENT_TEMPERATURE.getMeasure().getMeasurementUnit())) {
	...
	protected void testPowerLevel() {
		...
		try {
			...
			if (powerLevelSignal.getMeasure().getData() == WashingMachine.MAX_POWER_LEVEL.getData() &&
		...
		try {
			...
			powerLevelSignal = this.wmop.getCurrentPowerLevel();
			if (powerLevelSignal.getMeasure().getData() == WashingMachine.MAX_POWER_LEVEL.getData() / 2.0 &&
					powerLevelSignal.getMeasure().getMeasurementUnit().equals(
							WashingMachine.MAX_POWER_LEVEL.getMeasurementUnit())) {
	...
	protected void testSuspendResumeImmediateWashing() {
		...
		try {
			...
			if (stillNotWashing && this.wmop.on()) {
				...
				this.statistics.incorrectResult();
				this.logMessage("     but was during suspension: isWashing=" + this.wmop.isWashing() + ", on="
						+ this.wmop.on());
			}
	...
	protected void testSuspendResumeDelayedStart() {
		...
		try {
			...
			if (stillNotWashing && this.wmop.on()) {
				...
				this.statistics.incorrectResult();
				this.logMessage(
						"     but during suspension: isWashing=" + this.wmop.isWashing() + ", on=" + this.wmop.on());
			}
	...
	public synchronized void start() throws ComponentStartException {
		...
		try {
			...
			this.clock = clocksServerOutboundPort.getClock(CVMIntegrationTest.CLOCK_URI);
	...
	public synchronized void execute() throws Exception {
		if (this.isUnitTest) {
			...
			AcceleratedClock ac = clocksServerOutboundPort.getClock(
			...
			this.scheduleTaskOnComponent(
					new AbstractComponent.AbstractTask() {
						...
						public void run() {
							try {
								o.traceMessage("WashingMachine switches on.\n");
								wmop.switchOn();
							} catch (Throwable e) {
								e.printStackTrace();
							}
			...
			this.scheduleTaskOnComponent(
					new AbstractComponent.AbstractTask() {
						...
						public void run() {
							try {
								o.traceMessage("WashingMachine switches off.\n");
								wmop.switchOff();
							} catch (Throwable e) {
								e.printStackTrace();
							}
