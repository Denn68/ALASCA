...
implements	AdjustableCI
{
	 ...
	 *  6 (2000.0 watts).													*/
	public static final int		MAX_FAN_MODE = 6;
	/** the minimum admissible temperature from which the fan should
	 *  be resumed in priority after being suspended to save energy.		*/
	public static final double	MIN_FAN_ADMISSIBLE_TEMP = 12.0;
	/** the maximal admissible difference between the target and the
	 *  current temperature from which the fan should be resumed in
	 *  priority after being suspended to save energy.						*/
	public static final double	MAX_FAN_ADMISSIBLE_DELTA = 10.0;

	/** the current mode of the fan.										*/
	protected int		currentMode;
	/** true if the fan has been suspended, false otherwise.				*/
	protected boolean	isSuspended;

	public				FanConnector()
	{
		super();
		this.currentMode = MAX_FAN_MODE;
		this.isSuspended = false;
	}

	protected double	computePowerLevel(int mode) throws Exception
	{
		assert	mode > 0 || mode <= MAX_FAN_MODE :
				new PreconditionException("mode > 0 || mode <= MAX_MODE");

		double maxPowerLevel =
				((FanExternalControlJava4CI)this.offering).
													getMaxPowerLevelJava4();
		return (mode - 1) * maxPowerLevel/(MAX_FAN_MODE - 1);
	}
	...
	public boolean		upMode() throws Exception
	{
		...
		assert	this.currentMode() < MAX_FAN_MODE :
	...
	public double		emergency() throws Exception
	{
		...
		double ret = -1.0;
		if (currentTemperature < FanConnector.MIN_FAN_ADMISSIBLE_TEMP ||
							delta >= FanConnector.MAX_FAN_ADMISSIBLE_DELTA) {
			ret = 1.0;
		} else {
			ret = delta/FanConnector.MAX_FAN_ADMISSIBLE_DELTA;
		}
