...
public class KettleUnitTester
	...
	protected void initialise(
		...
		assert KettleUnitTester.implementationInvariants(this) : new ImplementationInvariantException(
	...
	protected void testTargetTemperature() {
		...
		try {
			...
			if (temperature.getMeasure().getData() == Kettle.STANDARD_TARGET_TEMPERATURE.getData()
		...
		try {
			...
			temperature = this.kop.getTargetTemperature();
			if (temperature.getMeasure().getData() == 85.0 &&
	...
	protected void testCurrentTemperature() {
		...
		try {
			...
			if (temperature.getMeasure().getData() == Kettle.FAKE_CURRENT_TEMPERATURE.getMeasure().getData() &&
					temperature.getMeasure().getMeasurementUnit().equals(
							Kettle.FAKE_CURRENT_TEMPERATURE.getMeasure().getMeasurementUnit())) {
	...
	protected void testPowerLevel() {
		...
		try {
			...
			if (powerLevelSignal.getMeasure().getData() == Kettle.MAX_POWER_LEVEL.getData() &&
		...
		try {
			...
			this.kop.setCurrentPowerLevel(
					new Measure<Double>(Kettle.MAX_POWER_LEVEL.getData() / 2.0,
							Kettle.POWER_UNIT));
			powerLevelSignal = this.kop.getCurrentPowerLevel();
			if (powerLevelSignal.getMeasure().getData() == Kettle.MAX_POWER_LEVEL.getData() / 2.0 &&
					powerLevelSignal.getMeasure().getMeasurementUnit().equals(
							Kettle.MAX_POWER_LEVEL.getMeasurementUnit())) {
