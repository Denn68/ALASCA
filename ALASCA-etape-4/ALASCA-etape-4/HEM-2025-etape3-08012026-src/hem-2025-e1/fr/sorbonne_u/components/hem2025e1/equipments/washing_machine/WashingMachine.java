...
public class WashingMachine extends AbstractComponent implements WashingMachineUserI, WashingMachineInternalControlI {
	...
	public void setTargetTemperature(Measure<Double> target) throws Exception {
		...
		assert getTargetTemperature().getMeasure().equals(target)
				: new PostconditionException("getTargetTemperature().getMeasure().equals(target)");
	}
	...
	public SignalData<Double> getTargetTemperature() throws Exception {
		...
		assert ret != null && TEMPERATURE_UNIT.equals(ret.getMeasure().getMeasurementUnit())
				: new PostconditionException(
						"return != null && TEMPERATURE_UNIT.equals(" + "return.getMeasure().getMeasurementUnit())");
		assert ret.getMeasure().getData() >= MIN_TARGET_TEMPERATURE.getData()
				&& ret.getMeasure().getData() <= MAX_TARGET_TEMPERATURE.getData()
				: new PostconditionException("return.getMeasure().getData() >= MIN_TARGET_TEMPERATURE.getData() "
